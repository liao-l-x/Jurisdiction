package com.llx.studio.util;

import com.alibaba.fastjson.JSONObject;
import com.llx.studio.config.exception.CommonJsonException;
...
public class CommonUtil {
	...
	public static JSONObject successJson(Object info,int totalCount) {
		JSONObject resultJson = new JSONObject();
		resultJson.put("code", Constants.SUCCESS_CODE);
		resultJson.put("msg", Constants.SUCCESS_MSG);
	 ...
	 */
	public static JSONObject successPage(final JSONObject requestJson, List<JSONObject> list, int totalCount,List<JSONObject> booten) {
		//设置每页多少条
		...
		if (null != booten && booten.size() != 0 ){
			info.put("bootrn",booten);
		}
		info.put("list", list);
	...
	public static JSONObject request2Json(HttpServletRequest request) {
		...
		requestJson.put("uri",contextPath);
		return requestJson;
	}
